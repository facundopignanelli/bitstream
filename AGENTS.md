...
- **Proactive implementation**: When the user reports an issue, asks why a bug/defect is occurring, or points out a visual or functional problem, do not just explain the cause and stop to ask for permission to apply the fix. Analyze the root cause, implement the fix directly, test/validate it, and then report back what was done.
