...
- **Official X / Twitter Embed Support for Rebits**: Upgraded X/Twitter Rebits to use responsive native card styling with verified author avatar, handle, exact timestamp, formatted tweet text, and direct X actions.
...
- **Inline Composer Expansions & Anchored Popovers**: Replaced separate modal dialogs for Media, Link/Rebit, Schedule, and Mood in the composer with streamlined inline expansions, direct native uploads, and anchored popover menus across desktop and mobile.
  ...
  - **Mood Popover**: Replaced the mood feelings modal with an anchored popover featuring a scrollable feelings grid, custom emotion creator, and Set/Clear actions.
...
- **Migrate Custom Moods Management to Settings Tab**: Relocated the custom moods creation, editing, reordering, and deletion list interface from the composer's feelings modal into a new "Custom Moods" settings tab, resolving a desync quirk where unsubmitted edits in the composer were lost on page reload. Retained a simple custom mood creation form in the composer for quick on-the-fly custom postings. Adjusted the settings tab bar styles to automatically scale down tab titles to fit cleanly in a single line, added a page reload upon saving moods to update the sidebar feelings widget and timeline immediately, and configured the custom moods editor list to be scrollable to keep the save actions always visible.
