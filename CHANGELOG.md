...
### Fixed
- **Live Photos & HEIC Composer Previews and Cropping**: Fixed composer media preview squares and the image cropper attempting to directly render raw HEIC/motion photo files (which fail to decode in web browsers, causing broken preview squares and preventing cropping). Previews and the cropper now accurately use the server-generated JPEG sub-sizes (`preview_url`), with backend crop coordinate normalization and JPEG conversion for cropped HEIC attachments.
